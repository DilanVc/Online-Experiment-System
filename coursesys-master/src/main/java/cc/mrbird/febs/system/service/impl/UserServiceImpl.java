...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    @Override
    public IPage<User> findUserDetailListByRole(User user, QueryRequest request) {
        if (StringUtils.isNotBlank(user.getCreateTimeFrom()) &&
                StringUtils.equals(user.getCreateTimeFrom(), user.getCreateTimeTo())) {
            user.setCreateTimeFrom(user.getCreateTimeFrom() + " 00:00:00");
            user.setCreateTimeTo(user.getCreateTimeTo() + " 23:59:59");
        ...
        Page<User> page = new Page<>(request.getPageNum(), request.getPageSize());
        page.setSearchCount(false);
        page.setTotal(baseMapper.countUserDetailByRole(user));
        SortUtil.handlePageSort(request, page, "USER_ID", FebsConstant.ORDER_ASC, false);
        return this.baseMapper.findUserDetailPageByRole(page, user);
    }
